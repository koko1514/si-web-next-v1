      ...
      <div className="container mx-auto px-4 md:px-8 relative z-10">
        ...
        >
          <Button
            ...
            className="rounded-full px-6 transition-all duration-300 font-semibold cursor-pointer"
          >
          ...
          <Button
            ...
            className="rounded-full px-6 transition-all duration-300 font-semibold flex items-center gap-2 cursor-pointer"
          >
            <Trophy className="w-4 h-4 text-yellow-400" />
            Prestasi Mahasiswa ({galleryItems.filter(i => i.type === "achievement").length})
          </Button>
          <Button
          ...
          >
            ...
            Kegiatan & Event ({galleryItems.filter(i => i.type === "event").length})
          </Button>
        </motion.div>
          ...
          <AnimatePresence>
            {filteredItems.map((item, idx) => (
              <motion.div
                ...
                className="break-inside-avoid bg-card border border-border/60 rounded-2xl overflow-hidden shadow-md hover:shadow-2xl hover:border-accent/30 transition-colors duration-300 flex flex-col mb-6 relative group cursor-pointer"
              >
                {/* Image Frame */}
      ...
      <AnimatePresence>
        {isModalOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/85 backdrop-blur-sm cursor-zoom-out"
          >
            <button
              onClick={() => setIsModalOpen(false)}
