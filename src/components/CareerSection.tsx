        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-6">
          {careerRoles.map((item, index) => (
            <motion.div
              ...
              whileHover={{ y: -6 }}
              className="group relative p-4 sm:p-6 rounded-2xl bg-card border border-border hover:border-secondary/50 hover:shadow-xl transition-all duration-300 flex flex-col justify-between cursor-default"
            >
              <div>
                {/* Icon */}
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-secondary/10 flex items-center justify-center mb-3 sm:mb-4 group-hover:bg-secondary/25 transition-colors">
                  <item.icon className="w-5 h-5 sm:w-6 sm:h-6 text-secondary" />
                </div>
                {/* Role Title */}
                <h3 className="text-sm sm:text-lg font-bold text-foreground mb-1.5 sm:mb-2 font-space">
                  {item.role}
                </h3>
                {/* Role Description */}
              ...
              {/* Indicator (Always visible on mobile, hover-revealed on desktop) */}
              <div className="mt-3 sm:mt-4 flex items-center gap-1 text-[10px] sm:text-xs text-secondary font-medium opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
